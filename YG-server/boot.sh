...
# 		#  Since the init_db.sql script already contains the up to date tables
# 		# the upgrades done will cause errors. may need to remove current migrations and start the 
# 		# migrations over
# 		flask db upgrade
# 		if [[ "$?" == "0" ]]; then
# 				break
